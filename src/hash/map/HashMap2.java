        ...
        while (node != null) {
            ...
            if (hash1 > hash2) {
                ...
                cmp = -1;
                    ...
                    && (cmp = ((Comparable) key1).compareTo(key2)) != 0) {//hash值相等
            } else {
                cmp = System.identityHashCode(key1) - System.identityHashCode(key2);
            }
            if (cmp > 0) {
...
//        }
    ...
    private void fixAfterPut(Node<K, V> node) {
        ...
        if (isRed(uncle)) { //uncle是红色的话
            black(parent);
            black(uncle);
            fixAfterPut(red(grand));   //相当于上溢，就把grand染红当成新节点
            return;
        }
        //叔父节点不是红色
